...
data_files = sorted(os.listdir(os.path.join(ar_dir, "expt11-tNR-218389-300s")))
data_files = [
    os.path.join(ar_dir, "expt11-tNR-218389-300s", f)
    for f in data_files
    if f.endswith(".txt")
]
print(
    "Found %d data files in %s"
    % (len(data_files), os.path.join(ar_dir, "expt11-tNR-218389-300s"))
)

expt_list = []
for i, f in enumerate(data_files):
    _refl = np.loadtxt(os.path.join(ar_dir, "expt11-tNR-218389-300s", f)).T
